...
def parse_config():
    ...
    parser.add_argument("--weight_ldiv", type=float, default=0.00)
    parser.add_argument("--weight_udiv", type=float, default=0.040)
    ...
    train_file = "train_koniq_spaq_binary.txt"
    parser.add_argument("--train_txt", type=str, default=train_file) # train.txt | train_synthetic.txt | train_authentic.txt | train_sub2.txt | train_score.txt
    checkpoint_path = r"./checkpoint_" + train_file.replace(".txt", "") + "_head4_udiv0.040/"
...
if __name__ == "__main__":
    ...
    config.ckpt_best_path = os.path.join(config.ckpt, 'best')
